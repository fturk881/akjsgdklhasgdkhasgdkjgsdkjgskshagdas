...
.setDescription(`**__İstediğiniz Sunucunuyu Görüntülemek için !server <serverid>__ **(Server id sunucuların isimlerinin altında olan sayılardır.)
...
Data.add(Sunucu.id)
i++
OutPut += `**${i}.** » ${Sunucu.name}(\`${Sunucu.id}\`)${client.emojis.cache.get('828231264829833217')}(${Sunucu.members.cache.filter(member => member.voice.channel && !member.user.bot).size} \\🔉)\n`
})
}
