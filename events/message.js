...
module.exports = async message => {
  ...
  if (cmd) {
    ...
    > 
    > Detay İçin: [Tıkla!](https://discord.gg/nMkUkrTjQj)`).setThumbnail(client.user.avatarURL()).setFooter(message.author.username,message.author.avatarURL({dynamic:true}))).then(Bakımda => Bakımda.delete({timeout: 15000}))
    }
    client.channels.cache.get('851763748917346314').send(
    ...
    **Kullandığı Komut:** \`${command}\`
    ...
    .setThumbnail(message.author.avatarURL({dynamic:true}))
    .setDescription(`**Olamaz! Sen botun kara listesinde bulunuyorsun! Eğer kara listeden çıkmak istiyorsan [Destek Sunucusu](https://discord.gg/v5wMdJcVjd) üzerinden sahibime ulaş!**`)
  ...
  ${db.fetch('Kabul') || 1} Adet kişi kurallarımızı kabul etti!`)
    .setFooter(`Magama © ${client.user.username} Bot 2021`,client.user.avatarURL())
    ...
    if (Tepki.emoji.name === '✅') {
    Embed.delete()
    message.channel.send(`<@${message.author.id}> **Kurallarımızı kabul ettiniz! Lütfen kurallara uyun.**`)
